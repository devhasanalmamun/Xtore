...
// DND, Recursive file tree, Browser API's, Memoization, React core(Virtual dom, fiber tree), states, Promise, Lexical scope
...
export default function TestIndex() {
  const [columns, setColumns] = useState<Columns>({
    ...
    doing: [{ id: 1, title: 'card 2' }],
    done: [{ id: 1, title: 'card 3' }],
  })

  return (
    <section className="h-[100vh] bg-gray-50 py-16">
        ...
        {Object.entries(columns).map(([columnName, cards]) => {
          return (
            <div key={columnName} className="w-full space-y-6">
              ...
              {/* Cards Holder */}
              <div className="flex flex-col space-y-3 rounded bg-gray-100 p-4">
                {cards.map((card) => {
                  return (
                    <div key={card.id} className="rounded bg-gray-50 p-4 shadow" draggable>
                      {card.title}
                    </div>
                  )
                })}
